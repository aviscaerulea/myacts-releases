...
- エージェント向けスキル定義（SKILL.md）をバイナリに同梱し、`myacts skill` で出力
...
# エージェント向けスキル定義（SKILL.md）を Claude Code のスキルとして保存
mkdir -p ~/.claude/skills/myacts
myacts skill > ~/.claude/skills/myacts/SKILL.md
```

### グループアドレス展開

`myacts member` は、氏名解決できないアドレスをグループとみなし、Cloud Identity Groups API でメンバーを再帰的に展開します。入れ子のグループも展開し、深さの上限はありません。
